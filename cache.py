...
LastEditTime: 2025-03-23 08:37:58
LastEditors: 嘎嘣脆的贝爷
'''
import os
import json
...
class Cache:
    def __init__(self):
        ...
        if os.path.exists(resume_lock_name):
            with open(resume_lock_name, 'r', encoding='utf-8')as f:
                resume_lock = f.read()
        else:
            resume_lock = ''
        with open(Config.resume_name, 'r', encoding='utf-8')as f:
            self.resume = f.read()
        if self.resume != resume_lock or not os.path.exists('cache.json'):
            with open(resume_lock_name, 'w', encoding='utf-8')as f:
                f.write(self.resume)
